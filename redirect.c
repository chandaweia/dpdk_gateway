...
void printpkt(const struct rte_mbuf* bufs);
...
static const char *fmt="HTTP/1.1 302 Moved Temporatily\r\nLocation: %s\r\nContent-Length: 0\r\nConnection: close\r\n\r\n ";//without the last space,firefox will be fault.
...
/*static void showtcp(struct tcp_hdr *tcp)
{
	printf("tcp rx_win:%x\n",tcp->rx_win);
	printf("tcp tcp_urp:%x\n",tcp->tcp_urp);
}*/
...
static int my_tcp_listen_2_syn_recv(struct connection_state_item*conn_item,struct rte_mbuf *bufs,uint8_t port)
{
	...
	if(res2!=1){
		printf("call gethdr error\n");
		return -1;
    }

	//rte_memcpy(buf_send,bufs,sizeof(struct rte_mbuf));
	buf_send->nb_segs = 1;
	buf_send->next = NULL;
	buf_send->tso_segsz = 0;
	buf_send->port = bufs->port^1;	

	/*printf("buf_send->tso_segsz:%d,bufs->tso_segsz:%d\n",buf_send->tso_segsz,bufs->tso_segsz);
	printf("buf_send->port:%d,bufs->port:%d\n",buf_send->port,bufs->port);
	printf("bufs->buf_addr:%d,buf_len:%d,pool:%d\n",bufs->buf_addr,rte_be_to_cpu_16(bufs->buf_len),bufs->pool);
	printf("buf_send->buf_addr:%d,buf_len:%d,pool:%d\n",buf_send->buf_addr,rte_be_to_cpu_16(buf_send->buf_len),buf_send->pool);*/

	memset(eth_send,0,sizeof(*eth_send));
	...
	//add tcp options
	//char opt[12]={0x02,0x04,0x05,0xb4,0x01,0x01,0x04,0x02,0x01,0x03,0x03,0x08};
    ...
    *popt=tcp_opt;
    ...
    {
		memset(ip_send,0,sizeof(*ip_send));
		memcpy(ip_send,iphdr,sizeof(struct ipv4_hdr));

		//add 2017/8/10
        //ip_send->type_of_service = 0;
        //ip_send->packet_id = 0;
		//ip_send->fragment_offset = IP_DN_FRAGMENT_FLAG;

		ip_send->time_to_live = 128;
		//ip_send->total_length = htons(sizeof(struct ipv4_hdr)+sizeof(struct tcp_hdr)); //add option
		ip_send->total_length = rte_cpu_to_be_16(sizeof(struct ipv4_hdr)+sizeof(struct tcp_hdr)+12); //add option
		...
		tcp_send->data_off =0x80;  //not add option
		popt=(TCP_OPT *)(rte_pktmbuf_mtod(buf_send, char *) +sizeof(struct ether_hdr)+sizeof(struct ipv4_hdr)+sizeof(struct tcp_hdr));
		*popt=tcp_opt;
		rte_memcpy(popt,&tcp_opt,sizeof(TCP_OPT));
		//tcp_send->data_off = tcphdr->data_off;  //add option
		//rte_memcpy((char *)tcp_send+sizeof(struct tcp_hdr),opt,12);
		//printf("sizeof(*opt):%d,sizeof(opt)\n",sizeof(*opt),sizeof(opt));
		
		tcp_send->cksum = 0;
        tcp_send->cksum = rte_ipv4_udptcp_cksum(ip_send,tcp_send);
		conn_item->buf_send->data_len = sizeof(struct ether_hdr)+sizeof(struct ipv4_hdr)+sizeof(struct tcp_hdr)+12;//add option
		conn_item->buf_send->pkt_len = sizeof(struct ether_hdr)+sizeof(struct ipv4_hdr)+sizeof(struct tcp_hdr)+12;//add option
	...
	else if(ether_type == ETH_P_IPV6)
    {
		...
		tcp_send->data_off = 0x80;

		conn_item->buf_send->data_len = sizeof(struct ether_hdr)+sizeof(struct ipv6_hdr)+sizeof(struct tcp_hdr)+12;
		conn_item->buf_send->pkt_len = sizeof(struct ether_hdr)+sizeof(struct ipv6_hdr)+sizeof(struct tcp_hdr)+12;

		popt=(TCP_OPT *)(rte_pktmbuf_mtod(buf_send, char *) +sizeof(struct ether_hdr)+sizeof(struct ipv6_hdr)+sizeof(struct tcp_hdr));
		*popt=tcp_opt;
		rte_memcpy(popt,&tcp_opt,sizeof(TCP_OPT));
		tcp_send->cksum = 0;
	...
	#if 0
	memset(buf_send,0,sizeof(struct rte_mbuf));
	memcpy(buf_send,bufs,sizeof(struct rte_mbuf));
	dpdk_forward(buf_send,port^1);//for test
	#endif
...
{
	//printf("in my_tcp_establish_http_reply\n");
		struct rte_mbuf *buf_send=NULL;
        struct ether_hdr *ethhdr=NULL,*eth_send=NULL;
        struct ipv4_hdr *iphdr=NULL,*ip_send=NULL;
        struct ipv6_hdr *ip6hdr=NULL,*ip6_send=NULL;
	...
	else if(ether_type == ETH_P_IPV6)
    {
		...
		ip6_send->vtc_flow = ip6hdr->vtc_flow;
        ip6_send->hop_limits=128;
        ip6_send->payload_len=rte_cpu_to_be_16(sizeof(struct tcp_hdr)+redirect_buff_len);
		//printf("ip6_send->payload_len:%d\n",sizeof(struct tcp_hdr)+redirect_buff_len);
        ip6_send->proto=IPPROTO_TCP;
		...
		buf_send->data_len = hdrlen+redirect_buff_len;
		buf_send->pkt_len = hdrlen+redirect_buff_len;
	...
	conn_item->last_send_ack_seq=conn_item->last_recv_seq+bufs->pkt_len-hdrlen;//add redirect_buff	
	time(&conn_item->time);
	dpdk_forward(conn_item->buf_send,port);
...
{
	//printf("in my_tcp_establish_2_fin_wait1\n");
	struct rte_mbuf *buf_send=NULL;
        struct ether_hdr *ethhdr=NULL,*eth_send=NULL;
        struct ipv4_hdr *iphdr=NULL,*ip_send=NULL;
        struct ipv6_hdr *ip6hdr=NULL,*ip6_send=NULL;
	...
	tcp_send->src_port = tcphdr->dst_port;
        tcp_send->dst_port = tcphdr->src_port;
        tcp_send->sent_seq = rte_cpu_to_be_32(conn_item->last_send_seq);
        tcp_send->recv_ack = rte_cpu_to_be_32(conn_item->last_recv_seq+1);//原tcp_send->recv_ack = rte_cpu_to_be_32(conn_item->last_recv_seq);
        tcp_send->data_off = 0x50;
        //tcp_send->tcp_flags |= TCP_FIN_FLAG; //fin=1
        tcp_send->tcp_flags |= TCP_ACK_FLAG; //ack=1
		tcp_send->tcp_urp = 0;
        tcp_send->rx_win=rte_cpu_to_be_16(65535);
		//printf("tcp_send->sent_seq:%x,tcp_send->recv_ack:%x\n",conn_item->last_send_seq,conn_item->last_recv_seq+1);
	...
	if(ether_type == ETH_P_IP)
	...
	else if(ether_type == ETH_P_IPV6)
        {
				...
				ip6_send->vtc_flow = ip6hdr->vtc_flow;
                ip6_send->hop_limits=128;
                ip6_send->payload_len=rte_cpu_to_be_16(sizeof(struct tcp_hdr));
                ip6_send->proto=IPPROTO_TCP;
...
//客户端返回ack，四次挥手完成
static int my_tcp_recv_ack(struct connection_state_item*conn_item,struct rte_mbuf *bufs)
{
...
static int my_tcp_fin_wait2_2_timewait(struct connection_state_item*conn_item,struct rte_mbuf*bufs,uint8_t port)
{
	...
	//补充tcp报头
		...
		tcp_send->src_port = tcphdr->dst_port;
        tcp_send->dst_port = tcphdr->src_port;
        //tcp_send->sent_seq = rte_cpu_to_be_32(tcphdr->sent_seq);
        //tcp_send->recv_ack = rte_cpu_to_be_32(tcphdr->recv_ack+1);
		tcp_send->sent_seq = rte_cpu_to_be_32(conn_item->last_send_seq);
		tcp_send->recv_ack = rte_cpu_to_be_32(conn_item->last_recv_seq+1);
        tcp_send->data_off = 0x50;
        ...
        {
				memset(ip6_send,0,sizeof(struct ipv6_hdr));
				ip6_send->vtc_flow = ip6hdr->vtc_flow;
                ip6_send->hop_limits=128;
                ip6_send->payload_len=rte_cpu_to_be_16(sizeof(struct tcp_hdr));
                ip6_send->proto=IPPROTO_TCP;
...
static int my_tcp_fin_rst(struct connection_state_item*conn_item,struct rte_mbuf *bufs,uint8_t port)
{
	//printf("in my_tcp_fin_rst\n");
	struct rte_mbuf *buf_send=NULL;
    struct ether_hdr *ethhdr=NULL,*eth_send=NULL;
    struct ipv4_hdr *iphdr=NULL,*ip_send=NULL;
    struct ipv6_hdr *ip6hdr=NULL,*ip6_send=NULL;
	...
	tcp_send->src_port = tcphdr->dst_port;
    tcp_send->dst_port = tcphdr->src_port;
    //tcp_send->sent_seq = rte_cpu_to_be_32(conn_item->last_recv_ack_seq);
    //tcp_send->recv_ack = rte_cpu_to_be_32(tcphdr->sent_seq+1);//不需要吧?
	tcp_send->sent_seq = rte_cpu_to_be_32(conn_item->last_recv_ack_seq);
	tcp_send->recv_ack = rte_cpu_to_be_32(0);
	tcp_send->tcp_flags |= TCP_RST_FLAG;//rst=1
	tcp_send->tcp_flags &= ~TCP_ACK_FLAG;//ack=1
    tcp_send->tcp_urp = 0;
    tcp_send->rx_win=rte_cpu_to_be_16(65535);
	...
	if(ether_type == ETH_P_IP)
	{
		...
		tcp_send->cksum = 0;
        tcp_send->cksum = rte_ipv4_udptcp_cksum(ip_send,tcp_send);
        buf_send->data_len = sizeof(struct ether_hdr)+sizeof(struct ipv4_hdr)+sizeof(struct tcp_hdr);
        buf_send->pkt_len = sizeof(struct ether_hdr)+sizeof(struct ipv4_hdr)+sizeof(struct tcp_hdr);
	...
	else if(ether_type == ETH_P_IPV6)
	{
		...
		tcp_send->data_off = 0x50;
		buf_send->data_len = sizeof(struct ether_hdr)+sizeof(struct ipv6_hdr)+sizeof(struct tcp_hdr);
        buf_send->pkt_len = sizeof(struct ether_hdr)+sizeof(struct ipv6_hdr)+sizeof(struct tcp_hdr);
...
{
	//printf("@@@packet is transfered into redirect_packet\n");
	...
	if(conn_item==NULL){//
		printf("conn_item is null,discard_packet\n");
		goto discard_packet;
	}

	if(tcphdr->tcp_flags&TCP_RST_FLAG)//如果你得到的数据段设置了rst位，那说明你这一端有了问题。所以关闭该端
	{
		//printf("rst was set,discard_packet\n");
		dpdk_forward(bufs,port^1);
		conn_item->stat=TCP_CLOSE;
		goto discard_packet;
	}
	...
	switch(conn_item->stat)//policy:重发，状态迁移，忽略
	{
		case TCP_LISTEN:
			...
			//printf("\n!!!!!!!!!!!!!!!!!!!TCP_LISTEN!!!!!!!!!!!!!!!\n");
			if((tcphdr->tcp_flags&TCP_FIN_FLAG)&&(tcphdr->tcp_flags&TCP_ACK_FLAG))
			{
				//printf("fin,ack\n");
				my_tcp_fin_rst(conn_item,bufs,port);//rst=1, TCP_CLOSE
			}
			else if((tcphdr->tcp_flags&TCP_PSH_FLAG) &&  (buflen>0))
			{
				my_tcp_fin_rst(conn_item,bufs,port);//rst=1, TCP_CLOSE
			}
			else if((tcphdr->tcp_flags&TCP_SYN_FLAG)&&(!(tcphdr->tcp_flags&TCP_ACK_FLAG)))
			{
				/*conn_item->type=type;
			...
			else //其它情况直接断开连接，比如用户断网之后仍然存在之前的流
			{
				//goto discard_packet;
				my_tcp_fin_rst(conn_item,bufs,port);//rst=1, TCP_CLOSE
			}
		...
		case TCP_SYN_RECV: 
			...
			if(recv_seq==conn_item->last_recv_seq && recv_ack_seq ==conn_item->last_recv_ack_seq){
				//printf("重新发送\n\n");
				dpdk_forward(conn_item->buf_send,port);
				//dpdk_forward(conn_item->buf_send,port^1);//for test
				break;
			}
		...
		case TCP_ESTABLISHED:  
			...
			//int pkt_len=rte_pktmbuf_pktlen(bufs);
			//buflen=bufs->pkt_len - hdrlen;
			//printf("recv_seq:%x,conn_item->last_send_ack_seq:%x\n",recv_seq,conn_item->last_send_ack_seq);
			//printf("recv_ack_seq:%x, conn_item->last_send_seq:%x\n",recv_ack_seq,conn_item->last_send_seq);
			//printf("tcphdr->tcp_flags:%x,(tcphdr->tcp_flags&TCP_ACK_FLAG):%x,(tcphdr->tcp_flags&TCP_PSH_FLAG):%x,(tcphdr->tcp_flags&TCP_FIN_FLAG):%x\n",tcphdr->tcp_flags,(tcphdr->tcp_flags&TCP_ACK_FLAG),\
						(tcphdr->tcp_flags&TCP_PSH_FLAG),(tcphdr->tcp_flags&TCP_FIN_FLAG));
			/*printf("hdrlen:%d,buflen:%d,datalen:%d\n",hdrlen,buflen,bufs->data_len);*/
			if(recv_seq==conn_item->last_send_ack_seq && recv_ack_seq==conn_item->last_send_seq  && (tcphdr->tcp_flags&TCP_ACK_FLAG)){
				//printf("tcphdr->tcp_flags:%x,TCP_PSH_FLAG:%x,tcphdr->tcp_flags&TCP_PSH_FLAG:%d\n",tcphdr->tcp_flags,TCP_PSH_FLAG,tcphdr->tcp_flags&TCP_PSH_FLAG);
				/*if(  (tcphdr->tcp_flags&TCP_PSH_FLAG) && (buflen>0)  ){
					printf("will enter http reply\n");
				...
				//printf("buflen:%d\n",buflen);
				if((tcphdr->tcp_flags&TCP_FIN_FLAG) && (buflen==0))//fin,ack 客户端主动断开,先回复ack，再回复fin,ack
				{
					//printf("receive fin,ack---return ack and return fin,ack\n");
					my_tcp_establish_2_fin_wait1(conn_item,bufs,port);
					//my_tcp_fin_rst(conn_item,bufs,port);//rst=1, TCP_CLOSE
				}
				else if((!(tcphdr->tcp_flags&TCP_PSH_FLAG)) &&  (buflen==0)) //客户端发送ack,对重定向url的回应，方便记录ing...
				{
					//printf("receive ack\n");
				...
				/*   网关主动断开
				if((tcphdr->tcp_flags&TCP_FIN_FLAG) && (buflen==0))
                {
                    printf("receive fin,ack---return rst\n");
					my_tcp_fin_rst(conn_item,bufs,port);//rst=1, TCP_CLOSE
				}
				else if(  (!(tcphdr->tcp_flags&TCP_PSH_FLAG)) &&  (buflen==0)  ){    //客户端回复ack后，网关主动断开....然而会出错....
					//printf("my_tcp_establish_2_fin_wait1 fin=1,ack=1\n");
					printf("receive ack, return fin,ack\n");
					my_tcp_establish_2_fin_wait1(conn_item,bufs,port);
				}
				...
				/*else
				{
					//printf("add fragment length\n");
					//conn_item->last_send_ack_seq += bufs->pkt_len-hdrlen; //tcp分片
				}*/
			...
			break;
		case TCP_RECV_ACK: //客户端返回ack，四次挥手完成
			my_tcp_recv_ack(conn_item,bufs);
			break;
		case TCP_FIN_WAIT1:
...
static void conn_table_check_timeout(int sig)
{
	if(SIGALRM == sig)
	{
		struct connection_state_item * pcurrent = NULL;
...
int  init_cap_redirect(void)
{
	...
	if(result)
		return result;
	//set_redirect_url("http://ipv6.tsinghua.edu.cn/");	
	return 0;
}
...
void redirected_skb_enqueue(struct rte_mbuf*bufs,uint8_t port,struct rte_mempool *pool)
{
	...
	if(res == 2)//udp --discard
	{
		goto free_packet;
	}
	else if(res == 3 || res ==0) //icmp or else --directly transfer
	{
		goto pass_packet;
	}
	else if(res!=1)//error packet
    {
    	printf("get head info failed\n");
        goto free_packet;
    }
...
/*void printpkt(const struct rte_mbuf* bufs)
{
	int i;
	u_char *p=NULL;
	memcpy(p,bufs,bufs->pkt_len);
	printf("######################pkt data##################\n");
	for(i=0; i<bufs->pkt_len; ++i)
	{
		printf(" %02x",bufs[i]);
		if((i+1)%16 == 0)
		{
...
//for test
void redirected(struct rte_mbuf*bufs,uint8_t port,struct rte_mempool *pool)
{
	//printf("redirected\n");
	struct rte_mbuf *r = rte_pktmbuf_alloc(pool);
	memcpy(r,bufs,sizeof(struct rte_mbuf));
